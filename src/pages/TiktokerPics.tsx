...
import { digitalLatinoApi, Country, Format, TiktokPicsData, Song } from "@/lib/api";
...
// Función adaptadora para convertir TiktokPicsData a Song
const adaptTiktokPicsToSong = (tiktokItem: TiktokPicsData): Song => {
    return {
        cs_song: tiktokItem.cs_song,
        song: tiktokItem.song,
        artists: tiktokItem.artists,
        label: tiktokItem.label || '',
        score: tiktokItem.views_total,
        rk: tiktokItem.rk,
        // Campos con valores por defecto
        spotify_streams_total: 0,
        tiktok_views_total: tiktokItem.views_total,
        ...
        length_sec: 0,
        crg: '',
        avatar: tiktokItem.image_url || '',
        url: '',
        spotifyid: ''
    };
};

// Componente para mostrar el formato de números grandes
...
export default function TiktokerPics() {
    ...
    // TikTok Pics data state
    const [tiktokPicsData, setTiktokPicsData] = useState<TiktokPicsData[]>([]);
    ...
    // Dropdown state
    const [openDropdown, setOpenDropdown] = useState<'country' | 'format' | null>(null);
    ...
    // Fetch TikTok pics data
    const fetchTiktokPicsData = useCallback(async () => {
        try {
            setLoadingData(true);

            const formatId = parseInt(selectedFormat) || 0;

            console.log('Fetching TikTok pics:', { formatId });
    ...
    }, [selectedFormat, fetchTiktokPicsData]);
    ...
    const filteredData = useMemo(() => {
        ...
        if (!chartSearchQuery.trim()) {
            return tiktokPicsData;
        }

        const query = normalizeText(chartSearchQuery);
        return tiktokPicsData.filter((item) => {
    ...
    }, [tiktokPicsData, chartSearchQuery]);
    ...
    const getFilteredOptions = (options: any[], searchQuery: string, type: 'country' | 'format') => {
    ...
    const handleOptionSelect = (value: string, type: 'country' | 'format') => {
        if (type === 'country') {
    ...
    const handleArtistDetailsClick = (row: TiktokPicsData) => {
        if (!user) {
    ...
    const handleRestrictedToggle = (index: number, row: TiktokPicsData) => {
        if (user?.role === 'ARTIST') {
            const normalize = (t: string) => t.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().trim();
            const myArtistName = normalize(user.allowedArtistName || "");
                                    ...
                                    {chartSearchQuery && (
                                        <div className="mt-2 text-xs text-slate-600 flex justify-between items-center px-1">
                                            <span className="font-medium">
                                                {filteredData.length} de {tiktokPicsData.length} resultados encontrados
                                            </span>
                            ...
                            filteredData.map((row, index) => (
                                ...
                                >
                                    <div className="grid grid-cols-9 items-center gap-1 sm:gap-2 pl-1 sm:pl-3 pr-1 sm:pr-3 py-1 sm:py-1.5">
                                        ...
                                        {/* Track Info */}
                                        <div className="col-span-5 sm:col-span-5 flex items-center gap-1 sm:gap-3">
                                        ...
                                        <div className="col-span-3 sm:col-span-3">
                                            <div className="relative bg-white/80 backdrop-blur-sm border border-white/60 rounded p-1 sm:p-2 shadow-xs group-hover:shadow-md group-hover:bg-white/90 transition-all">
                                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-2">
                                                    ...
                                                    {/* Versión desktop */}
                                                    <div className="hidden sm:flex sm:items-center sm:justify-between sm:gap-3 sm:flex-auto">
                                                        <ButtonBigNumber
                                                            name="Videos"
                                                            quantity={row.no_videos}
                                                            compact={false}
                                                        ...
                                                        <ButtonBigNumber
                                                            name="Vistas"
                                                            quantity={row.views_total}
                                                            compact={false}
                                                            className="bg-white/80 backdrop-blur-sm border border-white/60 rounded-xl p-1.5 shadow-sm group-hover:shadow-md group-hover:bg-white/90"
                                                        />
                                                        <ButtonBigNumber
                                                            name="Likes"
                                                            quantity={row.likes_total}
                                                            compact={false}
                                                            className="bg-white/80 backdrop-blur-sm border border-white/60 rounded-xl p-1.5 shadow-sm group-hover:shadow-md group-hover:bg-white/90"
                                                        />
                                                        <ButtonBigNumber
                                                            name="Coment"
                                                            quantity={row.comments_total}
                                                            compact={false}
                                                            className="bg-white/80 backdrop-blur-sm border border-white/60 rounded-xl p-1.5 shadow-sm group-hover:shadow-md group-hover:bg-white/90"
                                                        />
                                                        <ButtonBigNumber
                                                            name="Compart"
                                                            quantity={row.shares_total}
                                                            compact={false}
                                                            className="bg-white/80 backdrop-blur-sm border border-white/60 rounded-xl p-1.5 shadow-sm group-hover:shadow-md group-hover:bg-white/90"
                                                        />
                                                    </div>
                    ...
                    <div className="mt-4 bg-gradient-to-r from-purple-50/80 via-blue-50/80 to-indigo-50/80 backdrop-blur-sm border border-purple-200/50 rounded-2xl p-4 shadow-lg">
                        <div className="text-center space-y-4">
                            <div className="flex justify-center items-center gap-3">
                                ...
                                <div>
                                    <h3 className="text-xl font-bold text-gray-900">
                                        ¿Quieres ver métricas detalladas de TikTok?
                                    </h3>
                                </div>
                            </div>
                            ...
                            <div className="grid gap-1 opacity-50 pointer-events-none">
                                {[
                                    { rank: 1, artist: "Artista Viral", track: "Canción Trending", views: "15.2M" },
                                    { rank: 2, artist: "TikTok Star", track: "Nuevo Hit", views: "12.8M" },
                                    { rank: 3, artist: "Influencer", track: "Canción Popular", views: "10.1M" }
                                ].map((item) => (
            ...
            {artistDetailsModal.isOpen && artistDetailsModal.artist && (
                <ChartArtistDetails
                    artist={{
                        ...
                        score: artistDetailsModal.artist.views_total || 0,
                        followers_total: 0,
                        monthly_listeners: 0,
                    }}
                    selectedCountry={selectedCountry}
