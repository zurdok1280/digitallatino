...
const AdminPanel = () => {
  ...
  const [roleChangeData, setRoleChangeData] = useState<{id: number, role: string} | null>(null);
 ...
 // const LOCAL_API_URL = "http://localhost:8085/admin/users";
  ...
  return (
    <div className="container mx-auto p-6 bg-white min-h-screen text-black relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-600">
            Gestiona los usuarios de la plataforma, agrega, cambia sus roles o elimínalos del sistema.
          </p>
        </div>
        <button 
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {users.map((u) => {
                ...
                return (
                <tr key={realId} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="p-4">
                    <span className={`px-3 py-1 text-xs rounded-full inline-block
                      ${u.role === 'ADMIN' ? 'bg-gradient-to-r from-purple-600 to-pink-500 text-white font-bold' : 
                        u.role === 'PREMIUM' ? 'bg-purple-600 text-white font-bold' : 
                        u.role === 'ARTIST' ? 'bg-pink-500 text-white font-bold' :     
                  ...
                  <td className="p-4 flex gap-2 justify-center">
                    <select
                      ...
                      value={u.role}
                      onChange={(e) => setRoleChangeData({ id: realId as number, role: e.target.value })}
                    >
      ...
      <AlertDialog open={userToDelete !== null} onOpenChange={() => !isDeleting && setUserToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta acción no se puede deshacer. Esto eliminará permanentemente la cuenta del usuario y removerá todos sus datos de nuestros servidores.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel 
      ...
      <AlertDialog open={roleChangeData !== null} onOpenChange={() => !isUpdatingRole && setRoleChangeData(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="text-gray-900">
            ...
            <AlertDialogDescription>
              Estás a punto de cambiar el rol de este usuario a <span className="font-bold text-black">{roleChangeData?.role}</span>. ¿Deseas continuar?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel 
